...
<nav class="navbar navbar-default" role="navigation">
	<div class="navbar-header">
		<a class="navbar-brand" href="#">听----故事管理</a>
	</div>
	<div>
		<button type="button" class="btn btn-danger navbar-btn listButton" onclick="deletelistenStory('listenstory')"><i class="fa fa-trash-o" aria-hidden="true"></i> 删除</button>
		...
		<button type="button" class="btn btn-success navbar-btn listButton" onclick="openAddData('addListenStory')"><i class="fa fa-plus" aria-hidden="true"></i> 新建</button>
		<form class="navbar-form navbar-right listSearch" role="search" method="get" action="/manage.php/Story/listenStoryList">
...
<div class="list-box">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><?php echo ($vo["listen_story_name"]); ?></td>
				<td><?php echo ($vo["sCategory"]["scategory_name"]); ?></td>
				<td><?php echo ($vo["sAlbum"]["salbum_name"]); ?></td>
				<td><?php echo (date("Y-m-d H:i:s",$vo["listen_story_time"])); ?></td>
...
<script>
    //列表页面点击删除按钮
    function deletelistenStory(table){
        var delID = '';
