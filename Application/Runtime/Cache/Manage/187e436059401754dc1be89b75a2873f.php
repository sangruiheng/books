...
<nav class="navbar navbar-default" role="navigation">
	<div class="navbar-header">
		<a class="navbar-brand" href="#">讲----故事管理</a>
	</div>
	<div>
		<button type="button" class="btn btn-danger navbar-btn listButton" onclick="deleteData('tellingstory')"><i class="fa fa-trash-o" aria-hidden="true"></i> 删除</button>
		<button type="button" class="btn btn-info navbar-btn listButton" onclick="editData('addTellingStory')"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> 修改</button>
		<button type="button" class="btn btn-success navbar-btn listButton" onclick="openAddData('addTellingStory')"><i class="fa fa-plus" aria-hidden="true"></i> 新建</button>
		<form class="navbar-form navbar-right listSearch" role="search" method="get" action="/manage.php/Story/tellingStoryList">
...
<div class="list-box">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><?php echo ($vo["telling_story_name"]); ?></td>
				<td><?php echo ($vo["telling_story_author"]); ?></td>
				<td><?php echo ($vo["sCategory"]["scategory_name"]); ?></td>
				<!--<td><?php echo ($vo["sAlbum"]["salbum_name"]); ?></td>-->
				<td><?php echo (date("Y-m-d H:i:s",$vo["telling_story_time"])); ?></td>
...
<script>
    //列表页面点击删除按钮
    function deleteTellingStory(table){
        var delID = '';
        ...
        if(delID.length <= 0){
            ...
            $.showAsk({
                ...
                content : '确定要删除吗？',
                callback : function(){
                    $.post(APP+'/Story/deleteTellingStory',{delID : delID,table : table},function(){
                        reload();
                    });
                }
            });
