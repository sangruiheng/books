...
<nav class="navbar navbar-default" role="navigation">
	...
	<div>
		<button type="button" class="btn btn-default navbar-btn listButton" onclick="window.location.href = '/manage.php/Scategory/listenScategoryList'"><i class="fa fa-times" aria-hidden="true"></i> 返回听故事分类</button>
		<button type="button" value="<?php echo ($thisScategory["id"]); ?>" class="btn btn-danger navbar-btn listButton" onclick="deleteListenSalbum(this,'salbum')"><i class="fa fa-trash-o" aria-hidden="true"></i> 删除</button>
		<button type="button" class="btn btn-info navbar-btn listButton" onclick="editData('addListenSalbum')"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> 修改</button>
		<button type="button" value="<?php echo ($thisScategory["id"]); ?>" class="btn btn-success navbar-btn listButton" onclick="openAddListenSalbum(this,'addListenSalbum')"><i class="fa fa-plus" aria-hidden="true"></i> 新建</button>
		<form class="navbar-form navbar-right listSearch" role="search" method="get" action="/manage.php/Salbum/listenSalbumList">
...
<script>
    function openAddListenSalbum(obj,src){
    ...
    //删除
    function deleteListenSalbum(obj,table){
        var scategory_id = $(obj).attr('value');
        var delID = '';
        ...
        if(delID.length <= 0){
            ...
            $.showAsk({
                ...
                content : '确定要删除吗？',
                callback : function(){
                    $.post(APP+'/Salbum/delListenSalbum',{delID : delID,table : table},function(){
                        window.location.href=APP+'/Salbum/listenSalbumList?scategory_id='+scategory_id ;
                    });
                }
            });
        }
