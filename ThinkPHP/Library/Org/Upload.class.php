...
class Upload
{
	 ...
	 * @return Upload
	 */
	function __construct($allowFileSize=20000,$upDir='Uploads/appImg/',$sType='PIC')
	{
	...
	private function checkSize($size) {
		return !($size > $this->allowSize) || (0 == $this->allowSize);
	}
	/**
	...
	function SaveFile($fileField,$isArry=false,$i=0)
	{
		...
		else
		{
			...
			/* 检查文件大小 */
	        if (!$this->checkSize($_FILES[$fileField]['size'])) {
	            $this->errorMsg .= '请上传指定图片大小！';
	            return false;
	        }
	 ...
	 * @return string
	 */
	private function getDateDir($dataStyle=3){
		switch ($dataStyle)
		{
			Case 1:
				$sCreateDir = date("Y");
